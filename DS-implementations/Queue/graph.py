...
def graph():
    ...
    sim_points = 500
    x, y = produce_points("test-queue-array", datapoints=sim_points)

    # Stack from the standard library
    x2, y2 = produce_points("test-queue-std", datapoints=sim_points)

    # Cuadratic 
    # x2, y2 = produce_points("cuadratic", datapoints=sim_points)
    x3, y3 = produce_points("test-queue-linked-list", datapoints=sim_points)
    ...
    plt.title("Time required to enqueue and dequeue N elements")
